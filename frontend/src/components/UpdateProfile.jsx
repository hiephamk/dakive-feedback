...
const UpdateProfile = () => {
  ...
  const fetchAccount = async () => {
    ...
    try {
      ...
      console.error("Cannot fetch account", error);
      alert(t('error.cannot_fetch_account'));
    }
  };
                ...
                <Box key={account.id} width="80%" p={8}>
                  {accountId === account.id ? (
                      <VStack>
                        <Input
                          type="text"
                          placeholder={t('update_profile.phone_number')}
                          value={phone_number}
                          onChange={(e) => setPhoneNumber(e.target.value)}
                        />
                        <Input
                          type="date"
                          placeholder={t('update_profile.birth_date')}
                          value={birth_date}
                          onChange={(e) => setBirthDate(e.target.value)}
                        />
                        <Textarea
                          placeholder={t('update_profile.bio')}
                          value={bio}
                          onChange={(e) => setBio(e.target.value)}
                        />
                        {/* <Box>
                        ...
                        >
                          ...
                          <FileUpload.Dropzone>
                            ...
                            <FileUpload.DropzoneContent>
                              <Box>{t('update_profile.drag_drop_files')}</Box>
                              <Box color="fg.muted">{t('update_profile.file_types')}</Box>
                            </FileUpload.DropzoneContent>
                          </FileUpload.Dropzone>
                          <FileUpload.List />
                        </FileUpload.Root>
                        <Button onClick={() => handleUpdateProfile(account.id)}>{t('update_profile.update')}</Button>
                        {profile_img && (
                          <Box>
                              <strong>{t('update_profile.selected_file')}</strong> {profile_img.name}
                          </Box>
                      )}
                      </VStack>
                  ) : (
                    <Box>{t('update_profile.no_account')}</Box>
                  )}
                </Box>
              ))
            ) : (
              <Box>{t('update_profile.no_accounts_found')}</Box>
            )}
          </VStack>
        </VStack>
      </Center>
